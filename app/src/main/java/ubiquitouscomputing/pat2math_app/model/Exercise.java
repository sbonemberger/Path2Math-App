...
public class Exercise {
    ...
    public Feedback answer(double numeratorAnswer, double denominatorAnswer) {
        Feedback f = Feedback.Corrector.correctsSimplification(currentFraction, numeratorAnswer, denominatorAnswer, levelHint);

        if (f.isCorrectAnswer()) {
            levelHint = 0;

            int divisor = (int) (currentFraction.getNumerator() / numeratorAnswer);
    ...
    public String toString() {
        String s = "Fração inicial: " + initialFraction +
                "\nFração atual: " + currentFraction +
                "\nPontuação: " + (earnedPoints - lostPoints) +
                "\nErros: " + numErrors;
        ...
        if (isSolved)
            ...
            s += "\nO exercício está em desenvolvimento";
